...
ncbi = ete3.NCBITaxa()
ranks = ['class', 'phylum', 'order', 'family', 'genus', 'superkingdom', 'kingdom', 'species'] # ranks to include in the FigTree annotation
...
#        My code had a bit added on telling the range of the protein search area
#
        if '-' in taxon_name:
            taxon_split = taxon_name.split('_')
            taxon_name = '_'.join(taxon_split[0:len(taxon_split)-1])
...
def write_taxids(taxid, organism):
    ...
    # I like printing these just to see what's going on. Comment out if you like
...
if args.taxid is False:
    ...
    print('getting records from NCBI')
